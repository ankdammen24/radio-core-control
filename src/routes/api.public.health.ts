    ...
    handlers: {
      GET: async () => {
        const { data } = await supabaseAdmin
          .from("service_health").select("*").order("reported_at", { ascending: false }).limit(50);
        return Response.json({ ok: true, recent: data ?? [] });
      },
      POST: async ({ request }) => {
        const token = request.headers.get("x-stack-token") ?? "";
        const hash = createHash("sha256").update(token).digest("hex");
        const { data: tok } = await supabaseAdmin.from("stack_tokens").select("id,station_id,is_active").eq("token_hash", hash).maybeSingle();

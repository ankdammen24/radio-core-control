...
function StoragePage() {
  ...
  const { data, isLoading } = useQuery({ queryKey:["storage_locations"], queryFn: async () => (await supabase.from("storage_locations").select("*").order("name")).data ?? [] });
  ...
  return (
    <AppLayout title="Storage Settings" description="Local AzuraCast paths and future S3-compatible storage." actions={
      isEditor && (
      ...
      <Card className="overflow-hidden">
        <Table>
          <TableHeader><TableRow><TableHead>Name</TableHead><TableHead>Type</TableHead><TableHead>Path</TableHead><TableHead>Active</TableHead></TableRow></TableHeader>
          <TableBody>

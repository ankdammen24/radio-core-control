...
function derivePlaylistSync(p: {
  azuracast_playlist_id?: string | null;
  sync_dirty?: boolean | null;
}): SyncStatus {
...
function PlaylistsPage() {
  ...
  const playlists = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await database
        .from("playlists")
        .select("*, stations(name), playlist_assignments(id)")
  ...
  const toggle = useMutation({
    mutationFn: async ({ id, val }: { id: string; val: boolean }) => {
      const { error } = await database.from("playlists").update({ is_active: val }).eq("id", id);
      if (error) throw error;
    },
  ...
  const del = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await database.from("playlists").delete().eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => {
    ...
    : playlists.error
      ...
      : filtered.length === 0
        ? {
            ...
            hint: "Spellistor visas här när backend innehåller rotationer eller speciallistor.",
          }
        : { kind: "ready" as const };
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <Label>Type</Label>
              <Select
                value={form.playlist_type}
                onValueChange={(v) => setForm({ ...form, playlist_type: v })}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
          ...
          <div>
            ...
            <Input
              value={form.azuracast_playlist_id}
              onChange={(e) => setForm({ ...form, azuracast_playlist_id: e.target.value })}
            />
          </div>
          <div>
        ...
        <TableBody>
          {filtered.map((p: any) => (
            <TableRow key={p.id}>
              ...
              <TableCell>
                <SyncStatusBadge status={derivePlaylistSync(p)} compact />
              </TableCell>
              <TableCell>
                <Switch
                  checked={p.is_active}
                  disabled={!isEditor}
                  onCheckedChange={(v) => toggle.mutate({ id: p.id, val: v })}
                />
              </TableCell>
              <TableCell className="text-right">
                {isEditor && (
                  <ConfirmDialog
                    ...
                    trigger={
                      <Button variant="ghost" size="icon" title="Queue sync">

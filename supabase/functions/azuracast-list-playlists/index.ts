...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const res = await fetch(`${conn.base_url.replace(/\/$/, "")}/api/station/${conn.azuracast_station_id}/playlists`, { headers: { "X-API-Key": apiKey } });
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    return new Response(JSON.stringify({ ok: true, items: await res.json() }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
  } catch (e: any) {
